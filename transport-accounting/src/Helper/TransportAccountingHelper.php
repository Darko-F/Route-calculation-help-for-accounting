...
namespace Joomla\Module\TransportAccounting\Site\Helper;
...
class TransportAccountingHelper
{
    ...
    public function saveInvoiceAjax()
    {
        ...
        } elseif (preg_match($documentType === 'proforma' ? '/^PR-TA-(\d{2})-\d+$/' : '/^TA-(\d{2})-\d+(?:[-\s].+)?$/', $invoiceNumber, $matches)) {
        ...
        while (true) {
            ...
            $query = $db->getQuery(true)
                ->insert($db->quoteName('#__transport_accounting_invoices'))
        ...
        if ($sourceDocumentId > 0) {
            $query = $db->getQuery(true)
                ->update($db->quoteName('#__transport_accounting_invoices'))
    ...
    public function addDraftLineAjax()
    {
        ...
        $query = $db->getQuery(true)
            ->insert($db->quoteName('#__transport_accounting_invoice_draft_lines'))
            ->columns($db->quoteName($columns))
            ->values(implode(',', $values));
        $db->setQuery($query)->execute();
    ...
    public function deleteDraftLineAjax()
    {
        ...
        $db = self::db();
        $query = $db->getQuery(true)
            ->delete($db->quoteName('#__transport_accounting_invoice_draft_lines'))
    ...
    public function loadCustomerAjax()
    {
        ...
        $query = $db->getQuery(true)
            ...
            ->from($db->quoteName('#__transport_accounting_customers'))
    ...
    public function deleteCustomerAjax()
    {
        ...
        $db = self::db();
        $query = $db->getQuery(true)
            ->delete($db->quoteName('#__transport_accounting_customers'))
            ->where($db->quoteName('id') . ' = ' . $customerId);
        $db->setQuery($query)->execute();
    ...
    public function listInvoicesAjax()
    {
        ...
        $query = $db->getQuery(true)
            ...
            ->select('(SELECT ' . $db->quoteName('c.customer_code')
                . ' FROM ' . $db->quoteName('#__transport_accounting_customers', 'c')
            ...
            ->select('(SELECT COALESCE(SUM(' . $db->quoteName('p.amount') . '), 0) FROM '
                . $db->quoteName('#__transport_accounting_invoice_payments', 'p')
                . ' WHERE ' . $db->quoteName('p.invoice_id') . ' = ' . $db->quoteName('i.id') . ') AS ' . $db->quoteName('paid_amount'))
            ->from($db->quoteName('#__transport_accounting_invoices', 'i'));
    ...
    {
        Session::checkToken('post') or throw new RuntimeException('Invalid Joomla session token.');
        throw new RuntimeException('Invoice and pro forma deletion is available only in the Transport Accounting administrator component.');
    }
    ...
    private static function saveCustomer(array $payload): array
    {
        ...
        if ($existingId) {
            ...
            $query = $db->getQuery(true)
                ->update($db->quoteName('#__transport_accounting_customers'))
                ->set($sets)
            ...
            $query = $db->getQuery(true)
                ->insert($db->quoteName('#__transport_accounting_customers'))
                ->columns($db->quoteName($columns))
                ->values(implode(',', $values));
            $db->setQuery($query)->execute();
            $id = (int) $db->insertid();
    ...
    private static function draftLines(int $customerId, string $projectRef): array
    {
        ...
        $query = $db->getQuery(true)
            ...
            ->from($db->quoteName('#__transport_accounting_invoice_draft_lines'))
            ->where($db->quoteName('customer_id') . ' = ' . $customerId)
            ->where($db->quoteName('project_ref') . ' = ' . $db->quote($projectRef))
    ...
    private static function nextInvoiceNumber(int $year, string $documentType = 'invoice'): string
    {
        ...
        $prefix = $documentType === 'proforma' ? 'PR-TA-' . $shortYear . '-' : 'TA-' . $shortYear . '-';
        $query = $db->getQuery(true)
    ...
    private static function ensureCustomerAddressColumn(): void
    {
        self::ensureTables();
        $db = self::db();
        $table = $db->replacePrefix('#__transport_accounting_customers');
        $columns = [
    ...
    private static function ensureInvoiceCustomerColumns(): void
    {
        self::ensureTables();
        $db = self::db();
        $table = $db->replacePrefix('#__transport_accounting_invoices');
        $columns = [
            'customer_name' => [
    ...
    private static function ensureTables(): void
    {
        ...
        $db = self::db();
        $db->setQuery(
            "CREATE TABLE IF NOT EXISTS " . $db->quoteName('#__transport_accounting_customers') . " (
        ...
        $db->setQuery(
            "CREATE TABLE IF NOT EXISTS " . $db->quoteName('#__transport_accounting_invoices') . " (
              " . $db->quoteName('id') . " int unsigned NOT NULL AUTO_INCREMENT,
        ...
        $db->setQuery(
            "CREATE TABLE IF NOT EXISTS " . $db->quoteName('#__transport_accounting_invoice_draft_lines') . " (
              " . $db->quoteName('id') . " int unsigned NOT NULL AUTO_INCREMENT,
              " . $db->quoteName('customer_id') . " int unsigned NOT NULL,
              " . $db->quoteName('customer_code') . " varchar(64) NOT NULL,
              " . $db->quoteName('customer_name') . " varchar(255) NOT NULL DEFAULT '',
        ...
        $db->setQuery(
            "CREATE TABLE IF NOT EXISTS " . $db->quoteName('#__transport_accounting_invoice_payments') . " (
              " . $db->quoteName('id') . " int unsigned NOT NULL AUTO_INCREMENT,
        ...
        $longRouteColumns = [
            ...
            '#__transport_accounting_invoice_draft_lines' => ['line_label'],
        ];
        ...
        $customerTable = $db->replacePrefix('#__transport_accounting_customers');
