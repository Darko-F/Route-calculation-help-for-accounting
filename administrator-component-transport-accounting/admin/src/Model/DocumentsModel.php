...
namespace Topoweryou\Component\TransportAccounting\Administrator\Model;
...
class DocumentsModel extends ListModel
{
    ...
    protected function getListQuery()
    {
        ...
        $query = $db->getQuery(true)
            ...
            ->from($db->quoteName('#__transport_accounting_invoices', 'a'));
        ...
        if ($signatureImageUrl !== '' && !preg_match('#^(?:https?:)?//#i', $signatureImageUrl) && !str_starts_with($signatureImageUrl, 'data:')) {
            ...
            if (!str_starts_with($signatureImageUrl, '/')) {
                ...
                $signatureImageUrl = preg_match('#^(?:images|media|modules)/#i', $signatureImageUrl)
                    ...
                    : $siteRoot . '/modules/mod_transport_accounting/media/' . ltrim($signatureImageUrl, '/');
            }
        }
    ...
    public function recordPayment(int $invoiceId, string $paymentDate, float $amount, string $method, string $reference, string $note, string $kind = 'payment', ?float $advanceRate = null): int
    {
        ...
        try {
            ...
            if ($remaining <= 0.004) {
                throw new RuntimeException(Text::_('COM_TRANSPORT_ACCOUNTING_PAYMENT_ALREADY_PAID'));
            }
            if ($amount > $remaining + 0.004) {
                throw new RuntimeException(Text::sprintf('COM_TRANSPORT_ACCOUNTING_PAYMENT_EXCEEDS_REMAINING', number_format($remaining, 2, ',', '.')));
            }
    ...
    public function deleteDocuments(array $ids): int
    {
        ...
        if (count($documents) !== count($ids)) {
            throw new RuntimeException(Text::_('COM_TRANSPORT_ACCOUNTING_NOT_FOUND'));
        }
        ...
        foreach ($documents as $document) {
            ...
            if (($document['document_type'] ?? '') === 'proforma' && $convertedId > 0 && !isset($selected[$convertedId])) {
                throw new RuntimeException(Text::sprintf('COM_TRANSPORT_ACCOUNTING_DELETE_CONVERTED_ERROR', (string) $document['invoice_number']));
            }
        }
        ...
        try {
            $query = $db->getQuery(true)
                ->delete($db->quoteName('#__transport_accounting_invoice_payments'))
                ->where($db->quoteName('invoice_id') . ' IN (' . $idList . ')');
            $db->setQuery($query)->execute();

            $query = $db->getQuery(true)
                ->delete($db->quoteName('#__transport_accounting_invoices'))
                ->where($db->quoteName('id') . ' IN (' . $idList . ')');
            $db->setQuery($query)->execute();
