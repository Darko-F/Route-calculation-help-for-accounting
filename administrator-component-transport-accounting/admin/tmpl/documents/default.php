...
$canDelete = $identity->authorise('core.delete', 'com_transport_accounting');
$canEdit = $identity->authorise('core.edit', 'com_transport_accounting');
$methodLabels = [
    'bank_transfer' => Text::_('COM_TRANSPORT_ACCOUNTING_PAYMENT_METHOD_BANK_TRANSFER'),
    'cash' => Text::_('COM_TRANSPORT_ACCOUNTING_PAYMENT_METHOD_CASH'),
    'card' => Text::_('COM_TRANSPORT_ACCOUNTING_PAYMENT_METHOD_CARD'),
    'other' => Text::_('COM_TRANSPORT_ACCOUNTING_PAYMENT_METHOD_OTHER'),
];
$pdfText = [];
foreach ([
  ...
  <div class="row g-2 align-items-end mb-3">
    <div class="col-lg-3">
      ...
      <input type="search" name="filter_search" id="filter_search" class="form-control"
        ...
        placeholder="<?php echo Text::_('COM_TRANSPORT_ACCOUNTING_SEARCH_PLACEHOLDER'); ?>">
    </div>
    <div class="col-lg-2">
      <label class="form-label" for="filter_document_type"><?php echo Text::_('COM_TRANSPORT_ACCOUNTING_TYPE'); ?></label>
      <select name="filter_document_type" id="filter_document_type" class="form-select" onchange="this.form.submit()">
        <option value=""><?php echo Text::_('COM_TRANSPORT_ACCOUNTING_ALL_TYPES'); ?></option>
        <option value="invoice"<?php echo $this->state->get('filter.document_type') === 'invoice' ? ' selected' : ''; ?>><?php echo Text::_('COM_TRANSPORT_ACCOUNTING_INVOICE'); ?></option>
        <option value="proforma"<?php echo $this->state->get('filter.document_type') === 'proforma' ? ' selected' : ''; ?>><?php echo Text::_('COM_TRANSPORT_ACCOUNTING_PROFORMA'); ?></option>
      </select>
    </div>
    <div class="col-lg-2">
      <label class="form-label" for="filter_payment_status"><?php echo Text::_('COM_TRANSPORT_ACCOUNTING_PAYMENT_STATUS'); ?></label>
      <select name="filter_payment_status" id="filter_payment_status" class="form-select" onchange="this.form.submit()">
        <option value=""><?php echo Text::_('COM_TRANSPORT_ACCOUNTING_ALL_PAYMENT_STATUSES'); ?></option>
        <?php foreach (['unpaid', 'partially_paid', 'paid'] as $status) : ?>
          <option value="<?php echo $status; ?>"<?php echo $this->state->get('filter.payment_status') === $status ? ' selected' : ''; ?>><?php echo Text::_('COM_TRANSPORT_ACCOUNTING_' . strtoupper($status)); ?></option>
        <?php endforeach; ?>
      </select>
    </div>
    <div class="col-lg-2">
    ...
    <div class="col-lg-3 d-flex gap-2">
      ...
      <a class="btn btn-secondary" href="<?php echo Route::_('index.php?option=com_transport_accounting&view=documents&filter_search=&filter_document_type=&filter_payment_status='); ?>"><?php echo Text::_('JSEARCH_FILTER_CLEAR'); ?></a>
    </div>
  </div>
      ...
      <table class="table table-striped table-hover align-middle mb-0">
        <thead>
          <tr>
            ...
            <th><?php echo HTMLHelper::_('searchtools.sort', 'COM_TRANSPORT_ACCOUNTING_NUMBER', 'a.invoice_number', $listDirn, $listOrder); ?></th>
            <th><?php echo HTMLHelper::_('searchtools.sort', 'COM_TRANSPORT_ACCOUNTING_TYPE', 'a.document_type', $listDirn, $listOrder); ?></th>
            <th><?php echo HTMLHelper::_('searchtools.sort', 'COM_TRANSPORT_ACCOUNTING_CUSTOMER', 'a.customer_name', $listDirn, $listOrder); ?></th>
            <th class="text-end"><?php echo HTMLHelper::_('searchtools.sort', 'COM_TRANSPORT_ACCOUNTING_AMOUNT', 'a.total_amount', $listDirn, $listOrder); ?></th>
            <th><?php echo Text::_('COM_TRANSPORT_ACCOUNTING_PAYMENT_STATUS'); ?></th>
            <th><?php echo HTMLHelper::_('searchtools.sort', 'COM_TRANSPORT_ACCOUNTING_DATE', 'a.created_at', $listDirn, $listOrder); ?></th>
            <th><?php echo Text::_('COM_TRANSPORT_ACCOUNTING_ACTIONS'); ?></th>
          </tr>
        </thead>
        <tbody>
        <?php foreach ($this->items as $i => $item) :
            ...
            $documentStatus = $isConverted
                ? Text::sprintf('COM_TRANSPORT_ACCOUNTING_CONVERTED_TO', (string) $item->converted_invoice_number)
                : Text::_('COM_TRANSPORT_ACCOUNTING_STATUS_' . strtoupper((string) $item->document_status));
        ...
        ?>
          <tr>
            ...
            <td><span class="badge bg-<?php echo $isProforma ? 'info text-dark' : 'primary'; ?>"><?php echo Text::_($isProforma ? 'COM_TRANSPORT_ACCOUNTING_PROFORMA' : 'COM_TRANSPORT_ACCOUNTING_INVOICE'); ?></span></td>
            ...
            <td>
              ...
              <?php else : ?>
                <span class="badge bg-<?php echo $paymentBadge; ?>"><?php echo Text::_('COM_TRANSPORT_ACCOUNTING_' . strtoupper($paymentStatus)); ?></span><br>
                <small><?php echo Text::_('COM_TRANSPORT_ACCOUNTING_PAID_AMOUNT'); ?>: <?php echo number_format((float) $item->paid_amount, 2, ',', '.'); ?> EUR</small><br>
                <small><?php echo Text::_('COM_TRANSPORT_ACCOUNTING_REMAINING'); ?>: <?php echo number_format((float) $item->remaining_amount, 2, ',', '.'); ?> EUR</small>
              <?php endif; ?>
            </td>
        ...
        <?php endforeach; ?>
        <?php if (!$this->items) : ?>
          <tr><td colspan="8" class="text-center py-4"><?php echo Text::_('COM_TRANSPORT_ACCOUNTING_NO_DOCUMENTS'); ?></td></tr>
        <?php endif; ?>
        </tbody>
      </table>
    </div>
...
<dialog id="transport-accounting-payment-dialog" class="border-0 rounded shadow p-0" style="width:min(720px, calc(100vw - 2rem));">
  <div class="p-4">
    <div class="d-flex justify-content-between align-items-start gap-3 mb-3">
      <div><h2 class="h4 mb-1"><?php echo Text::_('COM_TRANSPORT_ACCOUNTING_ADD_PAYMENT'); ?></h2><div id="transport-accounting-payment-document-number" class="text-muted"></div></div>
      <button type="button" class="btn-close" aria-label="<?php echo Text::_('COM_TRANSPORT_ACCOUNTING_CLOSE'); ?>" onclick="document.getElementById('transport-accounting-payment-dialog').close()"></button>
    </div>
    <form action="<?php echo Route::_('index.php?option=com_transport_accounting&task=documents.recordPayment'); ?>" method="post" id="transport-accounting-payment-form">
      ...
      <div class="row g-3">
        ...
        <div class="col-md-6"><label class="form-label" for="transport-accounting-payment-reference"><?php echo Text::_('COM_TRANSPORT_ACCOUNTING_PAYMENT_REFERENCE'); ?></label><input class="form-control" type="text" name="payment_reference" id="transport-accounting-payment-reference" maxlength="255"></div>
        <div class="col-12"><label class="form-label" for="transport-accounting-payment-note"><?php echo Text::_('COM_TRANSPORT_ACCOUNTING_PAYMENT_NOTE'); ?></label><textarea class="form-control" name="payment_note" id="transport-accounting-payment-note" rows="2" maxlength="2000"></textarea></div>
      </div>
      <div class="alert alert-warning mt-3 mb-2"><?php echo Text::_('COM_TRANSPORT_ACCOUNTING_FURS_PAYMENT_WARNING'); ?></div>
      <div class="alert alert-info mb-3"><?php echo Text::_('COM_TRANSPORT_ACCOUNTING_MINIMAX_PAYMENT_NOTE'); ?></div>
      <h3 class="h6"><?php echo Text::_('COM_TRANSPORT_ACCOUNTING_PAYMENT_HISTORY'); ?></h3>
      <div id="transport-accounting-payment-history" class="mb-3"></div>
      <div class="d-flex justify-content-end gap-2"><button type="button" class="btn btn-secondary" onclick="document.getElementById('transport-accounting-payment-dialog').close()"><?php echo Text::_('COM_TRANSPORT_ACCOUNTING_CLOSE'); ?></button><button type="submit" class="btn btn-primary"><?php echo Text::_('COM_TRANSPORT_ACCOUNTING_SAVE_PAYMENT'); ?></button></div>
      <?php echo HTMLHelper::_('form.token'); ?>
    </form>
  </div>
</dialog>
...
function transportAccountingEscape(value) {
  const node = document.createElement('div'); node.textContent = String(value ?? ''); return node.innerHTML;
}
function transportAccountingMoney(value) { return Number(value || 0).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 }) + ' EUR'; }
function transportAccountingDate(value) {
  const match = String(value || '').match(/^(\d{4})-(\d{2})-(\d{2})/); return match ? `${match[3]}/${match[2]}/${match[1]}` : String(value || '');
}
function transportAccountingOpenPaymentDialog(button) {
  const data = transportAccountingPaymentData(button), dialog = document.getElementById('transport-accounting-payment-dialog');
  document.getElementById('transport-accounting-payment-document-number').textContent = data.invoice_number || '';
  document.getElementById('transport-accounting-payment-invoice-id').value = data.id || '';
  const today = new Date(), pad = value => String(value).padStart(2, '0');
  document.getElementById('transport-accounting-payment-date').value = `${today.getFullYear()}-${pad(today.getMonth() + 1)}-${pad(today.getDate())}`;
  const amount = document.getElementById('transport-accounting-payment-amount'); amount.value = Number(data.remaining_amount || 0).toFixed(2); amount.max = Number(data.remaining_amount || 0).toFixed(2);
...
async function transportAccountingPdfFont(doc) {
  ...
  for (const [file, style] of files) {
    const bytes = new Uint8Array(await (await fetch(TRANSPORT_ACCOUNTING_PAYMENT_FONT_BASE + file)).arrayBuffer());
...
async function transportAccountingPdfImageDataUrl(imageUrl) {
...
async function transportAccountingDrawPaymentSignature(doc, y, company) {
  const pageHeight = doc.internal.pageSize.getHeight();
  if (y > pageHeight - 72) { doc.addPage(); y = 28; }
  const x = 145;
  doc.setFont('NotoSans', 'normal'); doc.setFontSize(9); doc.text(`${TRANSPORT_ACCOUNTING_PAYMENT_TEXT.CONFIRMATION_SIGNATURE}:`, x, y);
  const signatureUrl = String(company.signature_image_url || '').trim();
  if (signatureUrl) {
    try {
      const dataUrl = await transportAccountingPdfImageDataUrl(signatureUrl);
...
async function transportAccountingPaymentConfirmationPdf(button) {
  const data = transportAccountingPaymentData(button);
  if (!window.jspdf?.jsPDF || !Array.isArray(data.payments) || !data.payments.length) return;
  const doc = new window.jspdf.jsPDF({ unit: 'mm', format: 'a4' }); await transportAccountingPdfFont(doc);
  const right = doc.internal.pageSize.getWidth() - 16, company = TRANSPORT_ACCOUNTING_PAYMENT_COMPANY || {};
  let y = 18;
  doc.setFont('NotoSans', 'bold'); doc.setFontSize(17); doc.text(TRANSPORT_ACCOUNTING_PAYMENT_TEXT.PAYMENT_CONFIRMATION, 16, y); y += 10;
  ...
  const statusLabel = data.payment_status === 'paid'
    ? TRANSPORT_ACCOUNTING_PAYMENT_TEXT.CONFIRMATION_PAID_LABEL
    : (data.payment_status === 'partially_paid' ? TRANSPORT_ACCOUNTING_PAYMENT_TEXT.CONFIRMATION_PARTIAL_LABEL : TRANSPORT_ACCOUNTING_PAYMENT_TEXT.UNPAID);
  ...
  detail(TRANSPORT_ACCOUNTING_PAYMENT_TEXT.INVOICE, data.invoice_number); detail(TRANSPORT_ACCOUNTING_PAYMENT_TEXT.CUSTOMER, data.customer);
  ...
  detail(TRANSPORT_ACCOUNTING_PAYMENT_TEXT.DUE_DATE, transportAccountingDate(data.due_date)); detail(TRANSPORT_ACCOUNTING_PAYMENT_TEXT.INVOICE_TOTAL, transportAccountingMoney(data.invoice_total));
  y += 5; doc.setFont('NotoSans', 'bold'); doc.text(TRANSPORT_ACCOUNTING_PAYMENT_TEXT.PAYMENT_HISTORY, 16, y); y += 7;
  const columns = { date: 16, method: 47, reference: 96, amountRight: right };
  doc.setFontSize(8); doc.text(TRANSPORT_ACCOUNTING_PAYMENT_TEXT.PAYMENT_DATE, columns.date, y); doc.text(TRANSPORT_ACCOUNTING_PAYMENT_TEXT.PAYMENT_METHOD, columns.method, y); doc.text(TRANSPORT_ACCOUNTING_PAYMENT_TEXT.PAYMENT_REFERENCE, columns.reference, y); doc.text(TRANSPORT_ACCOUNTING_PAYMENT_TEXT.PAYMENT_AMOUNT, columns.amountRight, y, { align: 'right' }); y += 3; doc.line(16, y, right, y); y += 6;
  doc.setFont('NotoSans', 'normal');
  data.payments.forEach(payment => {
    ...
    const noteLines = payment.note ? doc.splitTextToSize(`${TRANSPORT_ACCOUNTING_PAYMENT_TEXT.PAYMENT_NOTE}: ${String(payment.note)}`, 174) : [];
    if (y + rowHeight + noteLines.length * 4 + 2 > 270) { doc.addPage(); y = 20; }
    doc.text(transportAccountingDate(payment.date), columns.date, y); doc.text(methodLines, columns.method, y); doc.text(referenceLines, columns.reference, y); doc.text(transportAccountingMoney(payment.amount), columns.amountRight, y, { align: 'right' }); y += rowHeight;
    if (noteLines.length) { doc.text(noteLines, 20, y); y += noteLines.length * 4 + 2; }
  });
  if (y > 245) { doc.addPage(); y = 20; }
  y += 4; doc.setFont('NotoSans', 'bold'); detail(TRANSPORT_ACCOUNTING_PAYMENT_TEXT.TOTAL_PAID, transportAccountingMoney(data.paid_amount)); detail(TRANSPORT_ACCOUNTING_PAYMENT_TEXT.REMAINING, transportAccountingMoney(data.remaining_amount));
  y += 6; doc.setFont('NotoSans', 'normal'); doc.setFontSize(9); doc.text(doc.splitTextToSize(TRANSPORT_ACCOUNTING_PAYMENT_TEXT.CONFIRMATION_STATEMENT, 175), 16, y);
  y += doc.splitTextToSize(TRANSPORT_ACCOUNTING_PAYMENT_TEXT.CONFIRMATION_STATEMENT, 175).length * 4 + 8;
  await transportAccountingDrawPaymentSignature(doc, y, company);
  const footerY = doc.internal.pageSize.getHeight() - 16; doc.setFontSize(8); if (company.footer) doc.text(doc.splitTextToSize(String(company.footer), 150), doc.internal.pageSize.getWidth() / 2, footerY - 7, { align: 'center' }); doc.text(`${TRANSPORT_ACCOUNTING_PAYMENT_TEXT.CONFIRMATION_GENERATED}: ${transportAccountingDate(new Date().toISOString().slice(0, 10))}`, 16, footerY);
  const safeNumber = String(data.invoice_number || '').replace(/[^A-Za-z0-9._-]+/g, '-'); doc.save(`${TRANSPORT_ACCOUNTING_PAYMENT_TEXT.CONFIRMATION_FILENAME_PREFIX}-${safeNumber}.pdf`);
}
</script>
